...
library(survival)
survival.altered <- function(input){
  ...
  colnames(input)[last.col] <- "Subtype Survival Log-Rank p-val"
  return(input)
}
survival.unaltered <- function(input){
  last.col <- ncol(input)+1
  # For each gene in the summary table:
  for (k in 1:nrow(input)){
    gene <- input$`Hugo Symbol`[k]
  ...
  colnames(input)[last.col] <- "WT Survival Log-Rank p-val"
  return(input)
}
