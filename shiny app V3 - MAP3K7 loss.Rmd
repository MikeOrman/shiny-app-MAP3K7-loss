...
```{r}
summary.full <- gleason.altered(summary.concordance)
```
## DFS in primary subtype (TCGA data)
```{r}
summary.full <- survival.altered(summary.full)
```
...
```
# Identify hits by subtype-specific survival
```{r}
hits <- survival.unaltered(summary.full)
hits <- hits[hits$`Subtype Survival Log-Rank p-val` <= 0.05 & hits$`WT Survival Log-Rank p-val` >= 0.5 & hits$`-log10(Molecular FDR)` >= -log10(0.05),]
datatable(hits)
```